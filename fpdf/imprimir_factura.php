...
session_start();
check_login();
$fecha_pago=isset($_GET['fecha_pago'])?$_GET['fecha_pago']:null;
$idcliente=isset($_GET['idcliente'])?$_GET['idcliente']:null;
if($fecha_pago==false || empty($fecha_pago)==true){
	header('location: ../index.php');
	exit();
} 
$consulta="SELECT * FROM facturacion INNER JOIN clientes ON facturacion.idcliente=clientes.idclientes WHERE facturacion.fecha_pago='$fecha_pago' AND facturacion.idcliente='$idcliente'";
...
for($i=0;$i<count($actividades);$i++)	{
	$pdf->Cell(95,5,"     ".utf8_decode(ucwords($actividades[$i])),0,0,'L');	
	$pdf->Cell(95,5,"$ ".utf8_decode(dinero($valores[$i]))."     ",0,1,'R');
	$cnt=$cnt+5;
