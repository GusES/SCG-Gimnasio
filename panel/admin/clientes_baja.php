				...
				<div class="row">
					...
					<div class="col-12">
						...
						<nav class="d-flex flex-row my-1">
							...
							<div class="d-inline-flex mx-2">
								<div data-toggle="tooltip" data-placement="top">
									<a class="d-inline-flex text-primary font-weight-bold" href="clientes_baja.php">Inasistentes Definitivos</a>
                  ...
                  ?>
									<tr>
										...
										<td>
											<div style="width:0;height: 0;overflow: hidden;"><?=$row->ult_ingreso?></div>
											<?=fechaCastellano($row->ult_ingreso)?>
										</td>
										<td>
											<a class='btn btn-secondary btn-sm mb-1' href="php/estado_cliente.php?dni=<?=$row->dni?>&idcliente=<?=$row->idclientes?>&estado=<?=$row->estado?>&ult_ingreso=<?=$row->ult_ingreso?>" onclick="return confirmar('clientes_baja.php')">
												<?php
                          if($row->estado == 3){
                            echo "Habilitar";
                          } else {
                            echo "Suspender";
											...
											<a class="btn btn-secondary btn-sm mb-1" href='php/eliminar_cliente.php?idcliente=<?=$row->idclientes?>' onClick="return confirmar('clientes_baja.php')">Eliminar</a>
										</td>
									</tr>
									<?php } ?>
								</tbody>
