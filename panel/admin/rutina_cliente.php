...
if(isset($_GET['idcliente']) && isset($_GET['idrutina'])){
	...
	$sql = "SELECT * FROM clientes INNER JOIN rutinas ON clientes.idclientes = rutinas.idcliente WHERE idclientes = '$idcliente' AND idrutinas = '$idrutina'";
	$rs = $con->query($sql);
	$row = $rs->fetch_object();
} else {
	mysqli_close($con);
	header('location:rutinas_por_clie.php'); 
} ?>
<!DOCTYPE html>
<html lang="es">
			...
			<div class="col-10">
				<div class="row">
					<div class="col-12 mb-2">
						<div class="row bg-secondary" style="height:28.9688px;">
							<div class="col-6">
						...
						<div class="row">
							<div class="col-4">
								<h5 class="m-0">Rutina de: <?=ucwords($row->nombre)?> <?=ucwords($row->apellido)?></h5>
							</div>
							<div class="col-4">
								<p class="m-0"><span style="font-weight:500">Asignada:</span> <?=fechaCastellano($row->fecha)?></p>
							</div>
							<div class="col-4">
								<p class="m-0"><span style="font-weight:500">Tipo:</span> <?=ucwords($row->tipo)?></p>
							</div>
							<div class="col-12">
								<a class="btn btn-sm btn-outline-secondary" href="rutinas_por_clie.php?idcliente=<?=$row->idclientes?>">Volver</a>
								<a class="btn btn-sm btn-outline-secondary" href="../../fpdf/imprimir_rutinas.php?idcliente=<?=$idcliente?>&idrutina=<?=$idrutina?>" target="_blank">Imprimir</a>
								<a class="btn btn-sm btn-outline-secondary" href="../../codigo_qr/index_qr.php?idcliente=<?=$idcliente?>&idrutina=<?=$idrutina?>" target="_blank">Qr Code</a>
							</div>
						</div>
					</div>
					<div class="col-12">
						<div class="table-responsive">
							<table class="table table-striped table-bordered" style="font-size:12px">
								...
								<tbody>
									<?php
										$comprobar="comprobar";
										$query="SELECT * FROM rutinas WHERE idrutinas = $idrutina and idcliente = $idcliente";
